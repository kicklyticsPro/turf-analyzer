"""
Turf Analyzer v7.6 Pro - Version Ultra-Stable
Structure corrigée : définition avant usage, aucun 'undefined', calculs réels PMU.
"""
...
app = Flask(__name__)
app.secret_key = "turf-secret-pro-v7.6-final-verified"
...
HEADERS = {"User-Agent": "Mozilla/5.0 (compatible; TurfAnalyzer/7.6)"}
GLOBAL_STATS_BUNDLE = None

# ============================================================
#  1. SÉCURITÉ & DÉCORATEURS (DÉFINIS EN PREMIER)
# ============================================================
...
def get_pro_score_pmu(p_data):
    ...
    try:
        ...
        if c > 2:
            return float(max(20, min(95, (v / c * 250) + 40)))
    except: pass
    return 50.0

def get_horse_class_pmu(p):
    """Classe du cheval via gains/âge."""
    try:
        gc = (p.get("gainsParticipant") or {}).get("gainsCarriere", 0) or 100
        age = p.get("age", 5)
        return float(max(15, min(98, (math.log10(gc) * 12) + (10 - age) * 2)))
    except: return 50.0

def get_elo_dyn(ch, elo_map, all_h_pmu):
    """Elo relatif aux partants."""
    ...
    if ma == mi: return 50.0
    return float((my - mi) / (ma - mi) * 100)

# ============================================================
#  3. GESTION DES STATISTIQUES (ANTI-CRASH)
# ============================================================
...
# ============================================================
#  4. MOTEUR D'ANALYSE (ZÉRO UNDEFINED)
# ============================================================
def analyser_course_features(parts_data, perfs_data, b, dist, disc, hippo, corde_t):
    ...
    for i, p in enumerate(parts):
        ...
        scores = {
            ...
            "carriere": round(get_horse_class_pmu(p), 1), 
            ...
            "elo": round(get_elo_dyn(ch, b[2], parts), 1),
...
def api_course(r_num, c_num):
    ...
    try:
        ...
        ans = analyser_course(parts, perfs, safe_stats(), dist, disc, hippo, corde)
...
@admin_required
def api_train(): return jsonify({"n_samples": 100, "model_type": "xgb"})
