...
# CONFIGURATION ÉLITE
HISTORY_DAYS = 30 # Limité pour la stabilité RAM du VPS
WINDOW_SHORT = 20
...
def safe_compute_stats(max_days=HISTORY_DAYS):
    """
    Bouclier Anti-Crash : Garantit le retour d'un tuple de 6 éléments, même vide,
    évitant l'erreur 'cannot unpack non-iterable NoneType object'.
    """
    try:
...
def score_forme_enrichi(perfs, today=None):
    ...
    for p in perfs[:6]:
        try:
            ...
            else: pts += 20 # Disqualifié ou autre
            valid_count += 1
        except: continue
...
# ============================================================
#  MACHINE LEARNING (35 Variables / v7.2)
# ============================================================
def featurize(p, nb_partants, avgs=None):
    """Vecteur de 35 features incluant interactions et valeurs relatives."""
    s = p["scores"]
    # 1-20: Scores Heuristiques
...
def _fetch_course_full(args):
    try:
        ...
        return (parts, args[3], args[4], args[5], args[0], args[6])
    except: return None
            ...
            for r in prog["programme"]["reunions"]:
                ...
                for c in r["courses"]:
                    if c.get("arriveeDefinitive"):
                        tasks.append((fmt_date(d), r["numOfficiel"], c["numOrdre"], c.get("discipline",""), hippo, delta, c.get("corde","")))
        except: continue

    with ThreadPoolExecutor(max_workers=5) as ex:
        results = [r for r in list(ex.map(_fetch_course_full, tasks)) if r]

    for parts_data, discipline, hippo, delta_days, date_str, corde_type in results:
...
def analyser_course_features(parts_data, perfs_data, team_stats, horse_stats, elo):
    ...
    for i, p in enumerate(parts):
        ...
        s_form_ec = get_form_ecurie_score(en, team_stats.get("entraineurs", {}))
        ...
        ans.append({
            ...
            "scores": {
                "marche": 10, "forme": round(s_f, 1), "carriere": 50, "gains": round(s_g, 1), "driver": get_bucket_score(team_stats.get("drivers",{}).get(p.get("driver"))) or 50, 
                ...
                "profile_match": 50, "musique": s_musique, "gains_relatifs": round(s_gains_rel, 1), "form_ecurie": round(s_form_ec, 1)
            },
...
def analyser_course(parts_data, perfs_data, bundle, use_ml=False, capital=100):
    ...
    if use_ml:
        ml_model, calib = load_ml_model(), load_calibration()
        if ml_model:
            nb = len(ans)
            avg = {"gains": sum(a["scores"]["gains"] for a in ans)/nb, "elo": sum(a["scores"]["elo"] for a in ans)/nb}
            raw_preds = []
            for a in ans:
                p_raw = ml_model.predict_one(featurize(a, nb, avg))
                if calib: p_raw = Calibrator.from_dict(calib).apply(p_raw) if isinstance(calib, dict) else p_raw
                raw_preds.append(p_raw)
            s_ml = sum(raw_preds) or 1
            for i, a in enumerate(ans):
                a["chanceML"] = round(raw_preds[i]/s_ml*100, 2)
    ...
    for i, a in enumerate(ans):
        ...
        a["valueBet"], a["isGold"], a["isCoupSur"] = is_val, (is_val and a["scores"]["form_ecurie"] > 60), (pl3[i] >= 65 and a["rang"] == 1 and a["scores"]["forme"] > 80)
...
def api_course(r_num, c_num):
    ...
    try:
        ...
        ans = analyser_course(parts, perfs, bundle, use_ml, cap)
        return jsonify({"date": d_str, "course": {"libelle": "Course R"+str(r_num)+"C"+str(c_num), "heure": "00:00"}, "analyses": ans, "ml_active": use_ml and load_ml_model() is not None})
    except Exception as e: return jsonify({"error": str(e)}), 500
...
def api_train():
    try:
        res = train_ml_model(days_back=min(int(request.args.get("days", 15)), 30))
...
def api_scan_alerts():
    ...
    try:
        ...
        alerts = []
        if not prog: return jsonify({"alerts": [], "config": config})
        for r in prog["programme"]["reunions"]:
            for c in r["courses"]:
